	...
	while (true) {
		...
		case OpCode::Halt: {
			...
			for (uint32_t i = 0; i < sp; ++i) {
				fprintf(stderr, "[%u] u32 = %u | f32 = %f\n", i, ctx->stack[i].u32, ctx->stack[i].f32);
			}
			mp = ~uint32_t(0);
			ip = ~uint32_t(0);
			break;
		...
		case OpCode::CallInModule: {
			...
			cp += 1;
			ctx->callstack_ret_module[cp] = mp;
			ctx->callstack_ret_address[cp] = ip + 1;
			ctx->callstack_ret_bp[cp] = bp;
			ctx->callstack_argc[cp] = argc;
			// Jump to function
			mp = i_module;
		...
		case OpCode::Ret: {
			...
			cp -= 1;
			// TODO: Pop args, Push return value
			break;
		}
		...
		case OpCode::StoreLocal: {
			...
			fprintf(stderr, "[DEBUG] | i_local = %u\n", i_local);
		...
		case OpCode::AddI32: {
			...
			pop_n(ctx, &sp, operands, 2);
			Value result = {};
			result.i32 = operands[0].i32 + operands[1].i32;
			push(ctx, &sp, result);
